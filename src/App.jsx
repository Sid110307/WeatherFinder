...
export default function App() {
    ...
    React.useEffect(() => {
        ...
        const getWeather = (latitude, longitude) => {
            axios(`https://api.weatherapi.com/v1/current.json?key=${process.env.REACT_APP_WEATHER_API_KEY}&q=`
                + `${latitude},${longitude}&aqi=yes&lang=${i18n.language}`)
                .then(r => {
                    ...
                    weather.aqi.co = parseFloat(r.data.current.air_quality.co).toFixed(2);
                    weather.aqi.o3 = parseFloat(r.data.current.air_quality.o3).toFixed(2);
                    weather.aqi.no2 = parseFloat(r.data.current.air_quality.no2).toFixed(2);
                    weather.aqi.so2 = parseFloat(r.data.current.air_quality.so2).toFixed(2);
                    weather.aqi.pm25 = parseFloat(r.data.current.air_quality.pm2_5).toFixed(2);
                    weather.aqi.pm10 = parseFloat(r.data.current.air_quality.pm10).toFixed(2);
                    ...
                    weather.aqi.epaIndex = epaIndexes[r.data.current.air_quality["us-epa-index"] - 1];
        ...
        const getGeolocation = () =>
            navigator.geolocation
                ...
                : showError({
                    message: t("Your browser doesn't support location services."),
                });
        ...
        temperatureValueRef.current.onclick = () => {
            ...
            if (weather.temp.unit === "celsius") {
                temperatureValueRef.current.innerHTML = `${Math.floor((weather.temp.value * 9) / 5 + 32)}°<span>F</span>`;
        ...
        setInterval(
            () =>
                blocksRef.current.childNodes.forEach(
                    (child, _) => (child.style.left = `${(Math.random() * 100) << 0}%`)
                ),
    ...
    return (
        <React.Fragment>
            <div className="blocks" ref={blocksRef}>
                {[...Array(15)].map((_, i) => <div key={i} className="block" />)}
            </div>
            <div className="container" ref={containerRef}>
                <div className="app-title">
                    <p>WeatherFinder</p>
                    <button className="settings-button" disabled={modalDisabled} onClick={() => setShowModal(true)}>
                        <span className="fas fa-cog" />
                    </button>
                </div>
                <div className="date-time">
                    <p>{dateTime.split(" ")[0]} {dateTime.split(" ")[1]}</p>
                </div>
                ...
                <div className="weather-container">
                    <div className="weather-icon">
                        <img
                            src={weather.dayNight === "unknown" ? unknownIcon : `https:${weather.iconId}`}
                            alt={t("Weather Icon")}
                        />
                    </div>
                    ...
                    <div className="data">
                        <p>{t("Nearest location")}: <span>{weather.city}, {weather.region}, {weather.country}</span></p>
                    </div>
                    <div className="data">
                        <p>{t("Coordinates")}: <span>{weather.coordinates || "unknown"}</span></p>
                    </div>
                    <div
                        className="advanced-mode-btn-wrapper"
                        style={{transform: `translateY(${advancedMode ? "430px" : 0})`, zIndex: advancedMode ? 1 : 0}}
                    >
                        <button
                        ...
                        >
							<span
                                ...
                                style={{transform: `rotate(${advancedMode ? "180deg" : 0})`, marginRight: 10}}
                            />
                            <p>{t("Advanced mode")}</p>
                        </button>
                    </div>
                    <div
                        ...
                        style={{visibility: advancedMode ? "visible" : "hidden", opacity: advancedMode ? 1 : 0}}
                    >
                        <div className="data">
                            <p>
                                <span>{t("Wind")}</span>
                                <span>
									{weather.windSpeed} km/h ({weather.windDirectionDeg}° {weather.windDirection})
								</span>
                            </p>
                        </div>
                        <div className="data">
                ...
                <div
                    className="app-title"
                    style={{marginTop: advancedMode ? 480 : "clamp(20px, 50%, 50px)", fontSize: "0.5rem"}}
                >
                    <p>{t("Developed by Siddharth Praveen Bharadwaj")}</p>
                </div>
            </div>
            ...
            >
                <div className="modal-content">
                    <div className="modal-title">
                        <p>{t("Settings")}</p>
                        <button className="close-button" disabled={modalDisabled} onClick={() => setShowModal(false)}>
                    ...
                    <div className="modal-body">
                        <div className="modal-row">
                            ...
                            <div className="modal-input">
                                <select ref={languageRef} defaultValue={localStorage.getItem("language") || "en"}>
                    ...
                    <div className="modal-body modal-footer">
                        <p>
                            ...
                            <a href="https://weatherapi.com/" target="_blank" rel="noopener noreferrer">WeatherAPI</a>
                            {" "}|{" "}
                            <a
